...
class S3:
    ...
    @classmethod
    def s3_to_pandas(cls,key, header=None,_type='csv'):
        try:
    ...
    @classmethod
    def s3_to_pandas_with_processing(cls, key, header=None,_type='csv'):
        try:
            # get key using boto3 client
            obj = cls.s3.get_object(Bucket=cls.BUCKET, Key=key)
            gz = gzip.GzipFile(fileobj=obj['Body'])
            ...
            return pd.read_csv(io.StringIO(lines), header=header, dtype=str)
        except Exception as e:
            print(e)
            try:
                if _type=='csv':
